...
arraylist_t* pending_remote_requests; // = NULL
ABT_mutex pending_remote_requests_abt_sync;
...
int main(int argc, char* argv[])
{
    ...
    ABT_mutex_create(&pending_remote_requests_abt_sync);

    ABT_mutex_lock(pending_remote_requests_abt_sync);
    pending_remote_requests = arraylist_create(256);
...
static int unifyfs_exit(void)
{
    ...
    if (NULL != pending_remote_requests) {
        arraylist_free(pending_remote_requests);
        pending_remote_requests = NULL;
    }
    ABT_mutex_unlock(pending_remote_requests_abt_sync);
    ...
    ret = ABT_mutex_free(&pending_remote_requests_abt_sync);
    if (ret != ABT_SUCCESS) {
        LOGERR("Error returned from ABT_mutex_free(): %d", ret);
    }
